...
2. Generating grounded responses using a **local LLM** via Ollama.

### Key Features
- Automatic, interactive API documentation with **Swagger UI** (`/docs`) generated by FastAPI.
- High-performance backend served by the **Uvicorn ASGI server**.
- Supports both **local** and **Docker-based deployments**.
- Keeps data private by using a **local LLM** (no external API calls).
